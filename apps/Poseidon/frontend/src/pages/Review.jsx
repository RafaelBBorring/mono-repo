...
import {
  ...
  XCircle, SkipForward, RefreshCw, Loader2, Cpu
} from 'lucide-react'
import clsx from 'clsx'
import { RadialBarChart, RadialBar, PolarAngleAxis } from 'recharts'

// ── Agent Report Panel ──────────────────────────────────────────────────────────
function AgentReportPanel({ report }) {
            ...
            return (
              <div key={key} className="bg-slate-800/60 rounded-lg px-3 py-2.5">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className={clsx('text-xs font-mono', pct >= 70 ? 'text-emerald-400' : pct >= 30 ? 'text-amber-400' : 'text-rose-400')}>
                    {pct}%
                  </span>
                </div>
                <div className="w-full h-1 bg-slate-700 rounded-full overflow-hidden mb-1.5">
                  <div
                    className={clsx('h-full rounded-full transition-all', pct >= 70 ? 'bg-emerald-500' : pct >= 30 ? 'bg-amber-500' : 'bg-rose-500')}
                    style={{ width: `${pct}%` }}
                  />
                </div>
      ...
      {current ? (
        <div className="grid grid-cols-5 gap-6">
          ...
          <div className="col-span-2 space-y-4">
            ...
            <AgentReportPanel report={detail?.agent_report || current?.agent_report} />
