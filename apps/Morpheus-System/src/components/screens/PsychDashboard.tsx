...
export default function PsychDashboard() {
  ...
  const navItems = [
    { id: "home", label: "Início", icon: <Sparkles size={18} />, active: section === "home", onClick: () => setSection("home") },
    { id: "agenda", label: "Agenda", icon: <CalendarRange size={18} />, active: section === "agenda", onClick: () => setSection("agenda") },
  ];

  const accountItems = [
    { icon: <Eye size={17} />, label: "Disponibilidade", onClick: () => setShowAvailModal(true) },
    { icon: <LogOut size={17} />, label: "Sair", onClick: () => { setView("splash"); setActivePsych(null); }, danger: true },
  ];
  ...
  return (
    ...
    <AppShell
      ...
      routeKey={section}
      navItems={navItems}
      accountSlot={<AccountPill displayName={p.shortName || p.name} items={accountItems} />}
      primaryAction={
        <Button variant="gradient" size="lg" onClick={() => openReservation({ date: selectedDate })}>
          <Plus size={20} /> Nova reserva
        </Button>
      }
    >
      {section === "home" ? (
        <main className="mx-auto grid max-w-7xl gap-8 px-6 py-10 sm:px-6 lg:grid-cols-[1fr_0.86fr] lg:gap-10 lg:px-10 lg:py-14">
          <section className="rounded-3xl premium-panel p-6 md:p-8 lg:p-10">
            ...
            <h2 className="mt-4 font-brand text-3xl font-semibold md:text-4xl lg:text-5xl">
              Sua agenda sem <span className="aurora-text">ruído.</span>
            </h2>
        ...
        <main className="mx-auto flex max-w-7xl flex-col gap-6 px-6 py-10 sm:px-6 lg:flex-row lg:gap-8 lg:px-10 lg:py-14">
          <aside className="rounded-3xl premium-panel p-5 md:p-6 lg:w-[380px] lg:shrink-0">
            <div className="mb-5 flex items-center justify-between">
              <button
                onClick={() => setCalMonth(subMonths(calMonth, 1))}
                className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[var(--border-medium)] bg-[var(--glass-soft)] text-[var(--text-muted)] transition hover:border-[var(--accent-lavender)] hover:text-[var(--text-primary)]"
          ...
          <button
            ...
            className="fixed bottom-6 left-6 z-30 inline-flex items-center gap-2 rounded-2xl border border-[var(--border-medium)] bg-[var(--bg-elevated)] px-4 py-3 font-body text-sm font-extrabold text-[var(--text-primary)] shadow-2xl transition hover:-translate-y-1 hover:border-[var(--accent-lavender)] sm:gap-3 sm:rounded-3xl sm:px-5 sm:py-4 lg:bottom-8"
          >
